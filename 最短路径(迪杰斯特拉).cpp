...
parent[]：最短路径上每个节点的前置节点列表
start：起点
*/
void Dijkstra(vector<vector<int>>& graph, vector<int>& dist, vector<int>& parent, int start)
{
	int len = graph.size();
	...
	for (int i = 1; i < len; ++i){
		...
		for (int j = 0; j < len; ++j){
			int newdist = (graph[index][j] == INT_MAX) ? INT_MAX : graph[index][j] + mindis;
